...
const taxonomyData = { products: [], brands: [], volunteers: [] };

const autocomplete = {
  volunteerName: {
    input: byId('volunteerName'), list: 'volunteers',
    message: 'Choose your name from the volunteer list.'
  },
  itemType: {
...
const volunteerReady = (async () => {
  ...
  try {
    ...
    if (!Array.isArray(volunteers)) throw new Error('Volunteer list is invalid.');

    taxonomyData.volunteers = volunteers;

    const currentName = state.input.value.trim().toLocaleLowerCase();
    if (currentName && volunteers.some(name => name.toLocaleLowerCase() === currentName)) {
      state.selected = true;
      state.input.setCustomValidity('');
    }
  } catch (error) {
    // Keep the repair form usable if the volunteer list cannot be reached.
    state.freeText = true;
    state.input.setCustomValidity('');
...
function resetAutocompletes() {
  for (const [kind, state] of Object.entries(autocomplete)) {
    cancelAutocomplete(state);
    state.selected = kind === 'volunteerName' &&
      taxonomyData.volunteers.some(
        name => name.toLocaleLowerCase() === state.input.value.trim().toLocaleLowerCase()
      );
    state.input.setCustomValidity('');
  }
...
form.addEventListener('submit', async event => {
  ...
  for (const state of Object.values(autocomplete)) {
    if (state.input.value.trim() && !state.selected && !state.freeText) {
